...
void print(const BigInteger x, ostream &out) {
    if (x.data.back() != 0 && !x.positive) out << "-";
    out << (x.data.empty() ? 0 : x.data.back());
    for (int i = (int)x.data.size() - 2; i >= 0; --i)
        out << setfill('0') << setw(BigInteger::DIGITS_COUNT) << x.data[i];
}

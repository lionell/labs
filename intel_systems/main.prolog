...
player_select_side(Side) :-
    ...
    (
        ...
        (write('Invalid side, please type \'black.\' or \'white.\''), nl, player_select_side(Side))
    ).
...
player_input_position(pos(X, Y)) :-
    ...
    (
        (pos(A, B), X = A, Y = B, !);
        (write('Invalid position. Retry: '), nl, player_input_position(pos(X, Y)))
    ).
...
player_input_move(Side, board(Black, White), NewBoard) :-
    ...
    (
        ...
        (write('Invalid move. Retry: '), nl, player_input_move(Side, board(Black, White), NewBoard))
    ), !.

% Let human make capture.
player_input_capture(Side, B1, B2) :-
    write('Please input your capture path (type \'end.\' to finish): '), nl,
    (
        (
        ...
        (write('Invalid capture. Retry:\n'), nl, player_input_capture(Side, B1, B2))
    ).

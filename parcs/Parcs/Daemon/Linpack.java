/**
 * Тест для определения производительности процессора путем решения системы
 * линейных уравнений
 */
public class Linpack {
  private int n;  //размер матрицы

  public Linpack () { n = 500; }
  public Linpack (int n) { this.n = n; }
  ...
  double second()
  {
   ...
   public double mflops_result = 0.0;
   public double residn_result = 0.0;
   public double time_result = 0.0;
   public double eps_result = 0.0;

   private double ops = 0.0, total = 0.0;

   /**
	* Запускает тест на интервал времени, примерно равный указанному
	* @param time время в миллисекундах
	*/
   public void run_benchmark (int time){
	   n = 300;
	   run_benchmark();
	   if (time/1000. <= total) return;
	   double ops1 = ops / total * (time/1000.-total);
	   n = (int) Math.pow(1.5*ops1, 1./3);
	   run_benchmark();
   }
  ...
  public void run_benchmark()
  {
	...
	ops += (2.0*((double)n*n*n))/3.0 + 2.0*(n*n);
	...
	residn_result += 0.005; // для округления
	residn_result = (int)(residn_result*100);
	residn_result /= 100;

	time_result += total;
	time_result += 0.005; // для округления
	time_result = (int)(time_result*100);
	time_result /= 100;

	mflops_result = ops/(1.0e6*total);
	mflops_result += 0.0005; // для округления
